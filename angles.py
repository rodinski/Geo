...
`Angle` is for representing generic angle. `AlphaAngle` is for
...
def normalize(num, lower=0, upper=360, b=False):
    ...
    if not b:
        ...
        if num < lower or num == upper: num = upper - abs(num - lower) % (abs(lower) + abs(upper))
...
if __name__ == "__main__":
    ...
    print("No Tests ran.")
    a = Angle(r=math.pi -0.04)
    a = Angle(r=0.04)
    print(f"{a}")
    b = Angle(r=(math.pi +0.05) )
    b = Angle(r=(2*math.pi - 0.05) )
    print(f"{b}")
    
    print(  f"{(a-b).r}     {normalize((a-b).r, -math.pi, math.pi, b=True)} " )
    print(  f"{(b-a).r}     {normalize((b-a).r, -math.pi, math.pi, b=True) }" )
    print(" = = = ")

    a = AlphaAngle(r=0.04)
    print(f"{a}")
    b = AlphaAngle(r=(2*math.pi -0.05) )
    print(f"{b}")
    ...
    move = d2r(end)-d2r(start)
    move = d2r(end-start)

    print(normalize( move, -pi, pi, False) )
    print(normalize( move, 0, 2*pi) )
